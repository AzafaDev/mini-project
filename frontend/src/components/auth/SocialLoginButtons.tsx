...
export const SocialLoginButtons: React.FC<SocialLoginButtonsProps> = ({
  ...
  return (
    <div className={`grid ${isRegister ? "grid-cols-2" : "grid-cols-2"} gap-4`}>
      <button
        type="button"
        onClick={onGoogleClick}
        className="flex items-center justify-center gap-3 h-11 rounded-lg bg-dark-elevated text-text-light text-sm font-semibold hover:bg-dark-card-hover transition-colors ring-1 ring-white/5"
      >
        <img
          alt="Google"
      ...
      <button
        ...
        className="flex items-center justify-center gap-3 h-11 rounded-lg bg-dark-elevated text-text-light text-sm font-semibold hover:bg-dark-card-hover transition-colors ring-1 ring-white/5"
      >
        <svg className="w-5 h-5 fill-text-light" viewBox="0 0 24 24">
